...
cmd:text('')
cmd:option('-data_base_dir', 'summary/documents_processed', [[The base directory of the document path in data-path. If the document path in data-path is absolute path, set it to /]])
cmd:option('-data_path', 'summary/all_stories-train.txt', [[The path containing data file names and labels. Format per line: document_path label_idx]])
cmd:option('-label_path', 'summary/all_stories_targ.txt', [[The path containing labels. Format per line: characters]])
cmd:option('-val_data_path', 'summary/all_stories-val.txt', [[The path containing validate data file names and labels. Format per line: document_path label_idx]])
...
function main()
    ...
    local train_data = DataGen(opt.data_base_dir, opt.data_path, opt.label_path, opt.max_encoder_fine_l_h, opt.max_encoder_fine_l_w, opt.max_decoder_l)
    ...
    if phase == 'train' then
        ...
        val_data = DataGen(opt.data_base_dir, opt.val_data_path, opt.label_path, opt.max_encoder_fine_l_h, opt.max_encoder_fine_l_w, opt.max_decoder_l)
